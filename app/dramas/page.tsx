...
import { Plus, Drama, Clock, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 md:px-10 py-12 md:py-16">
      <div className="flex items-end justify-between mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Dramas</h1>
      ...
      {dramas.length === 0 ? (
        <div className="text-center py-16">
          <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center mx-auto mb-3">
            <Drama className="w-5 h-5 text-muted-foreground/60" />
          </div>
          ...
          {dramas.map((drama, i) => (
            ...
            >
              <div className="w-9 h-9 rounded-xl bg-amber-500/10 flex items-center justify-center shrink-0 mt-0.5">
                <Drama className="w-4 h-4 text-amber-600" />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                  <span className="capitalize">{drama.type}</span>
                  {drama.authorId && (
                    <>
                      <span className="text-muted-foreground/30">&middot;</span>
                      <span>{drama.authorId.name}</span>
                    </>
                  )}
                </div>
